...
impl<N: Eq + Hash + Copy + Send + 'static> WatchWalker<N, Start> {
    ...
    pub fn new<G: IntoNodeIdentifiers<NodeId = N> + IntoNeighborsDirected>(
        ...
        for node in graph.node_identifiers() {
            // Bounds is completely arbitrary
        ...
        for node in graph.node_identifiers() {
            ...
            join_handles.push(tokio::spawn(async move {
                // We run in a loop and wait for either a dependency to trigger a run or for a package change event
                loop {
